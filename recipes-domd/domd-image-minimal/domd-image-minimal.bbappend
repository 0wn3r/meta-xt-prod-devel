...
do_install_append () {
    ...
    if [ -n "${XT_USE_VIS_SERVER}" ]; then
        find ${DEPLOY_DIR}/${PN}/ipk/aarch64 -iname "aos-vis_git*" -exec cp -f {} ${DEPLOY_DIR}/domd-image-minimal/images/${MACHINE}-xt \; && \
        find ${DEPLOY_DIR}/domd-image-minimal/images/${MACHINE}-xt -iname "aos-vis_git*" -exec ln -sfr {} ${DEPLOY_DIR}/domd-image-minimal/images/${MACHINE}-xt/aos-vis \; || true
    fi
}
